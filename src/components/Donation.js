...
export default function App () {
    ...
    const submitDona = async () => {
        ...
        // 응답 (실패)
            ...
            else if(sender_name === "" || sender_phone === "" || sender_address === ""){
        ...
        <div className='dona_home'>
            <div className='dona_container' style={{display: 'flex'}}>
                <div className='dona_direction'>
                    <span>중고 전자기기 선물하기</span><br/><br/>
                    우리집에 돌아다니는 중고 스마트폰을 기부해<br/>
                    경제적인 어려움을 느끼는 청소년들의 디지털 격차 해소에 도움을 주세요!<br/><br/>   
                    ⭕ 잠금 장치는 모두 해제해주시기 바랍니다.<br/>
                    ⭕ 도착 즉시 초기화를 진행하게 됩니다.<br/>
                    ⭕ 배송 후 접수 취소 및 초기화 복구는 불가능합니다.<br/>
                    ⭕ 기기에 중요한 정보가 없는지 확인해주시기 바랍니다.
                </div>
                <div className='round_container_pink'>
                    {/* 기종 선택 */}
                    <div style={{display:'flex'}}>
                        <div className='input_title'><span>* </span>기종</div>
                    ...
                    {/* 기기명 선택 */}
                    <div style={{display:'flex'}}>
                        <div className='input_title'><span>* </span>기기명</div>
                        <div className='input_box'>
                            <input placeholder='ex. 갤럭시 Z플립 3' onChange={(e) => { setModelName(e.target.value) }}/>
                        </div>
                    </div>
                    {/* 특이사항 입력 (선택)  */}
                    <div style={{display:'flex'}}>
                        <div className='input_title'><span>* </span>특이사항</div>
                        <div className='input_box'>
                            <span><textarea placeholder='ex. 블루투스 연결이 안 됨 / 액정 깨짐' onChange={(e) => { setRemarks(e.target.value) }}/></span>
                        </div>
                    </div>
                </div>
            </div>
            <div className='dona_container' style={{display: 'flex'}}>
                <div className='deliver_info'>보내는 분
                    <div className='round_container'>
                        <div style={{display:'flex'}}>
                            <div className='input_title'><span>* </span>성명</div>
                            <div className='input_box'>
                                <input placeholder='ex. 홍길동' onChange={(e) => { setSenderName(e.target.value) }}/>
                            </div>
                        </div>
                        <div style={{display:'flex'}}>
                            <div className='input_title'><span>* </span>전화</div>
                            <div className='input_box'>
                                <input placeholder='ex. ex. 010-1234-5678' onChange={(e) => { setSenderPhone(e.target.value) }}/>
                            </div>
                        </div>
                        <div style={{display:'flex'}}>
                            <div className='input_title'><span>* </span>주소</div>
                            <div className='input_box'>
                                <input placeholder='상세주소까지 입력' onChange={(e) => { setSenderAddress(e.target.value) }}/>
                            </div>
                        </div>
                        <div style={{display:'flex'}}>
                ...
                <div className='deliver_info'>받는 분
                    <div className='round_container'>
                        <div style={{display:'flex'}}>
            ...
            <div className='dona_text'>
                <span>※ 접수 후 7일 내로 발송 부탁드립니다. 선물이 잘 도착하면 입력하신 번호로 문자 드리겠습니다. 감사합니다.</span>
            </div>
            
            <div className='btn_container'>
                <button className='dona_btn' onClick={submitDona}>선물하기</button>
            </div>
